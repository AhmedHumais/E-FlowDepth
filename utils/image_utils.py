...
def forward_interp_disp(disp_in:torch.Tensor, flow_in: torch.Tensor):
    # Same as the numpy implementation, but differentiable :)
    # Disp: [B,1,H,W]
    disp = disp_in.clone()
    if len(disp.shape) < 4:
    ...
    # Note that a Nearest Neighbor Interpolation would be better. But there does not exist a pytorch fcn yet.
    # See issue: https://github.com/pytorch/pytorch/issues/50339
    disp_new = torch.zeros_like(disp, device=device)
    for i in range(b):
        disp_new[i,0] = grid_sample_values(torch.stack([x1[i],y1[i],d[i]]), h, w)[0]
